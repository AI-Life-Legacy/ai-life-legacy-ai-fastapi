...
*   **유저 필터링**: `filter={"user_id": user_id}`를 통해 해당 사용자의 데이터만 독립적으로 검색합니다. (사용자 간 데이터 유출 및 목업/샘플 데이터 간섭을 방지하기 위해 글로벌/공유 데이터나 타인 데이터의 Fallback 조회를 완전히 차단하고, 오직 요청한 사용자의 데이터만 가져오도록 고정합니다.)
