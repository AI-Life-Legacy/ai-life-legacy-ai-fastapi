...
def get_image_luminance(image_path: str):
...
def parse_markdown_autobiography(file_path: Path):
    ...
    for i, section in enumerate(sections[1:]):
        ...
        mood = mood_match.group(1).strip() if mood_match else detect_mood(chapter_title, body)
...
def paginate_to_spreads(data):
    ...
    for chapter_idx, chapter in enumerate(data['chapters']):
        ...
        brightness = get_image_luminance(opener_img) if has_bg_image else 255
        title_theme = "dark" if brightness > 100 else "light"
        title_size = get_title_size(chapter['chapter_title'])
        ...
        if has_bg_image and paras:
            ...
            while paras and current_lines < max_lines:
                p_lines = estimate_lines(paras[0])
...
def generate_pdf():
    ...
    raw_data = parse_markdown_autobiography(md_path)
    spread_data = paginate_to_spreads(raw_data)
...
<head>
    ...
    <style>
        ...
        /* --- Template 1: Chapter Opener (Case A & B) --- */
        .t1-opener {
            flex: 1;
            display: flex;
            flex-direction: column;
            position: relative;
            background-color: #fafafa; /* Base fallback */
            width: 304mm; /* Spread level container fix */
        }
        ...
        /* Layer 2: Overlay Layer (Protection) */
        .t1-overlay {
            position: absolute;
            inset: 0;
            z-index: 2;
        ...
        /* Layout logic for Image-less (Case B) */
        .t1-opener.no-image {
            background: linear-gradient(135deg, #fdfcfb 0%, #e2d1c3 100%); /* 따뜻한 베이지 그라데이션 fallback */
        }
        ...
        /* Layer 3: Typography Layer (Always Rendered) */
        .t1-typography {
            flex: 1;
            display: flex;
            flex-direction: column;
            ...
            position: relative;
            z-index: 3;
            max-width: 122mm; /* Ensure it stays in left page area */
        }
        ...
        .chapter-title {
            ...
            line-height: 1.3;
            word-break: keep-all; 
            overflow-wrap: break-word; /* Auto line break */
            max-width: 100%;
        }
        .chapter-sub {
            font-size: 10pt;
        ...
        /* --- Images --- */
        .image-full {
             width: calc(100% + 43mm); /* Covers bleeds */
        ...
        .t8-quote-text {
            font-size: 20pt; /* Reduced to fit longer quotes */
            color: #111;
            line-height: 1.8;
            text-align: center;
            font-weight: 700;
            position: relative;
            font-style: italic;
            word-break: keep-all; /* Prevent character-level splitting */
        }
    ...
    <div class="spread">
        ...
        <div class="page left">
            ...
            <div class="content" style="flex: 1; position: relative;">
                ...
                {% elif spread.type == 'T6' %}
                    <div class="t8-quote-box">
                        <div class="t8-quote-text">인생의 가장 아름다운 순간은<br>기록을 통해 영원해집니다.</div>
                    </div>
                {% else %}
...
"""
    ...
    output_path = PROJECT_ROOT / "storage" / "data" / "autobiography_book.premium.spread.pdf"
    
    # Render PDF
    HTML(string=rendered_html, base_url=".").write_pdf(target=output_path)
    
    print(f"Success: Premium Spread Autobiography PDF generated at: {output_path}")
